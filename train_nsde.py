...
def L_data(model, state, state_next, action, cum_r, cum_r_next, t0, t1, key):
    ...
    sigma_norm_squared_diff = diff @ jnp.linalg.inv(sigma) @ diff ### inv of diagonal matrix
...
def train_step(model, opt_state, batch, key):
    ...
    def loss(model):
        ...
        return  total_loss, (loss_data, loss_sc, loss_grad, loss_mu)
...
for epoch in range(epochs):
    ...
    wandb.log({"Loss data term": train_loss, 
                "Loss SC": individal_losses[1],
                "Loss Grad": individal_losses[2],
                "Loss Mu": individal_losses[3],
                 "State Test Rel l2 Error": test_state_err,
